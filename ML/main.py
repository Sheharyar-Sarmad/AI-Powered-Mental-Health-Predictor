...
# Countries the model was trained on. Anything else typed by the user gets
# bucketed into "Other" so the model always receives a category it knows.
top_countries = ['Other', 'India', 'USA', 'Canada', 'Australia', 'UK', 'Germany', 'Mexico', 'Turkey', 'France']

# Common aliases -> canonical label used by the model / top_countries list
COUNTRY_ALIASES = {
    'united states': 'USA',
...
def normalize_country(raw: str) -> str:
    """Map any free-text country input to one of the known top_countries."""
    if not raw:
        return 'Other'
    cleaned = raw.strip()
...
class StudentData(BaseModel):
    ...
    country: str = Field(..., description="Any country name; unrecognized values are bucketed into 'Other'.")
...
@app.get('/countries')
def get_countries():
    """Exposes the canonical country list so the frontend dropdown always
    stays in sync with what the backend/model actually supports."""
